...
double O1[V][O][O][O];
double O2[V][V][V][O];
double temp1[V][V][V][O][O][O];
double temp2[V][V][V][O][O][O];
      ...
      for(c=0;c<V;c++)
        for(i=0;i<O;i++)
          for(j=0;j<O;j++)
            for(k=0;k<O;k++)
                for(m=0;m<O;m++)
                  temp1[a][b][c][i][j][k] += V * T2[a][b][k][m]*O1[c][m][i][j];
  for(a=0;a<V;a++)
    for(b=0;b<V;b++)
      for(c=0;c<V;c++)
        for(i=0;i<O;i++)
          for(j=0;j<O;j++)
            for(k=0;k<O;k++)
                for(e=0;e<V;e++)
                  temp2[a][b][c][i][j][k] += O * T2[c][e][i][j]*O2[a][b][e][k];  
  for(a=0;a<V;a++)
    for(b=0;b<V;b++)
      for(c=0;c<V;c++)
        for(i=0;i<O;i++)
          for(j=0;j<O;j++)
            for(k=0;k<O;k++)
              X[a][b][c][i][j][k] = temp1[a][b][c][i][j][k] + temp2[a][b][c][i][j][k];
